...
<span class="token key">{prop}</span><span class="token punctuation">:</span> <span class="token {typeof $$props[prop] === 'object'? typeof $$props[prop][0] : typeof $$props[prop]}">{typeof $$props[prop] === 'object'? $$props[prop].join(', ') : $$props[prop]}</span><br/>{/each}{#if $$props.tags}<TagList tags={$$props.tags} caps={false} brackets={true}/>{/if}
</pre>
{/if}
